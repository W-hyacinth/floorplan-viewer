...
```jsonc
{
  ...
  "zones": [                      // 출입금지 구역 — 3D·미니맵에선 벽과 같은 막힌 덩어리로 보임(금지구역임을 드러내지 않음).
    { "id": "zone-abc", "x": 300, "z": 100, "w": 150, "d": 120 },  // 사각형 (에디터에서 드래그 지정)
    { "id": "zone-def", "points": [ { "x": 0, "z": 0 }, { "x": 200, "z": 0 }, { "x": 200, "z": 150 }, { "x": 80, "z": 150 }, { "x": 80, "z": 300 }, { "x": 0, "z": 300 } ] }
    // 다각형 (에디터에서 클릭=닫힌 영역 자동 감지, 꼭짓점 편집 가능). points가 있으면 x/z/w/d 무시.
    // 진입 불가 + 내부 가구는 상호작용·조명 비활성. 붉은 표시는 어드민 에디터 전용.
  ],
