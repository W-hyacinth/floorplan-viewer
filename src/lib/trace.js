...
const THIN_T_CM = 5        // 얇은 벽 경로 하한: 실전 도면은 6~8cm 내벽을 그린다
const THIN_LEN_CM = 150    // 얇은 벽은 길어야 인정(글자 가로획은 ~85cm를 못 넘는다 — 실측)
const THIN_FILL = 0.8      // 얇은 벽은 꽉 차 있어야 함
...
export async function detectWalls(src, underlay, debug = false) {
  ...
  for (const vertical of [false, true]) {
    ...
    for (const bd of bands) {
      ...
      // 얇은 벽 경로: 두께 5~9cm라도 길고 꽉 찬 띠는 벽(극세 내벽·보정 오차로 얇아진 내벽)
      const thinOk = tCm >= THIN_T_CM && lenCm >= THIN_LEN_CM && fill >= THIN_FILL
      const normalOk = tCm >= WALL_MIN_T_CM && (
        fill >= SOLID_FILL ||
        hollowOk() ||
