...
const MERGE_OPEN_FRAC = 0.6  // + 인터페이스 과반이 열려 있어야 병합(문은 벽 공유가 길어 미달)
...
export async function detectWalls2(src, underlay, debug = false) {
  ...
  if (debug) preMergePng = renderLabelPng(label, wall, w, h, N)
  ...
  for (let iter = 0; iter < 8; iter++) {
    ...
    for (const [k, pxs] of openByPair) {
      ...
      if (a < 2 || b < 2) continue
      // 인터페이스 과반이 진짜 벽이면 문으로 이어진 별개의 방이다(방 병합 금지)
      if (pxs.length / (pxs.length + (thickByPair.get(k) || 0)) < MERGE_OPEN_FRAC) continue
  ...
  return {
    ...
    maskPng,
    stats: {
      cmPerPx, rooms, keep: [...keep], segsDebug, pocketLog,
...
function measureThickness(wall, w, h, s) {
  ...
  // 35퍼센타일 — 중앙값은 리지에 접한 가구·수납 블롭이 두께를 60cm급으로 오염시킨다.
  // 진짜 벽은 표본이 균일해 퍼센타일에 둔감하고, 개구부/심볼 리지는 0이 많아 여전히 탈락.
